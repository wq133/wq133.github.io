...
# 部署jekyll（需直接引用，导入路径引用失败）--destination指定生成路径 --watch启用自动生成 nohup自动后台执行监听
# /usr/local/ruby/bin/bundle exec /usr/local/ruby/bin/jekyll build --destination=/var/wq133.github.io/_site --watch
